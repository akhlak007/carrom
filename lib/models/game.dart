import 'package:carrom_game/models/player.dart';
...
class Game {
  Game() {
    ...
    _currentPlayer = 1;
    _score = 0;
    _players = <Player>[];_players.add(Player());
    _players.add(Player());
  }
  ...
  List<Player> _players = []; // Initialize with an empty list
  ...
  bool _multiStrike(int scenario) {
    if (scenario == 2) {
      if (_blackCoins >= 1 && _redCoin == 1) {
        ...
        final int playerTurn = getCurrentPlayer() - 1;
        _score = getPlayerScore(playerTurn);
        _setPlayerScore(_score + 3, playerTurn);_players[playerTurn].setIdleCount(0);
        return _changePlayer();
      }
      return false;
    } else {
  ...
  bool _defunctCoin(int scenario) {
    if (scenario == 2) {
      ...
      if (_blackCoins >= 1) {
        _blackCoins--;
        final int playerTurn = getCurrentPlayer() - 1;
        _score = getPlayerScore(playerTurn);_setPlayerScore(_score - 2, playerTurn);
        _players[playerTurn].setFoulCount(_incrementFoulCount(playerTurn));
        _players[playerTurn].setIdleCount(_incrementIdleCount(playerTurn));
        return _changePlayer();
      }
